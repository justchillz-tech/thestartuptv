  ...
  useEffect(() => {
    ...
    async function loadSession() {
      ...
      if (sessionError || !data.session) {
        setError("This invitation link is invalid or has expired. Please ask the festival administrator to send a new invitation.");
      } else {
    ...
    <main className="auth-shell">
      ...
      <section className="auth-card">
        ...
        <div className="auth-heading">
          ...
          <p>Create your password to activate your jury account and access your assigned films.</p>
        </div>

        {!ready ? (
          <div className="auth-message">Checking your invitation…</div>
        ) : error && !email ? (
          ...
          <form onSubmit={handleSubmit} className="auth-form">
            ...
            <button className="button button-primary" disabled={loading}>
              {loading ? "Setting password…" : "Set Password"} <span>↗</span>
            </button>
          </form>
        )}
      </section>
      ...
      <style jsx>{`
        .auth-message{padding:14px;border:1px solid var(--glass-border);border-radius:12px;color:var(--text-secondary);font-size:11px;text-align:center}
      `}</style>
    </main>
  );
}
